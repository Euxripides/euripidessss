...
# 80. Windows / E 盘 ClickHouse 部署硬约束

> 本项目明确要求：**ClickHouse 安装及其持久化数据不得落在 C: 盘。**
> 推荐在 Windows 主机中使用 **WSL2 Linux / Docker Linux 容器**运行 ClickHouse，并确保承载 ClickHouse 的 WSL 虚拟磁盘、数据库目录、日志、临时目录、备份目录全部位于 E: 盘。
...
E:\database\clickhouse\
├── runtime\
│   └── wsl\                 # 推荐：承载 ClickHouse Linux 运行环境
├── data\                    # ClickHouse 数据
├── logs\                    # ClickHouse 日志
├── tmp\                     # 临时文件
├── user_files\              # file() / 导入文件
├── format_schemas\          # format schema
├── backups\                 # 本地备份
├── config\
│   ├── config.d\
│   └── users.d\
├── migration\               # 旧 Parquet → DB 迁移中间区
└── export_spool\            # 大型导出任务临时文件
```
...
```text
C:\clickhouse
C:\ProgramData\ClickHouse
C:\Users\<user>\.clickhouse
C:\Users\<user>\AppData\...\ClickHouseData
```
...
# 81. 推荐部署方式：WSL2 发行版直接放 E 盘

为了确保 ClickHouse 的 Linux 程序文件和数据库数据都真正位于 E 盘，优先推荐：
...
WSL2
  ↓
独立 Linux Distribution
  ↓
E:\database\clickhouse\runtime\wsl\
  ↓
ClickHouse Server
```

不要直接使用默认安装在 C 盘的 WSL 发行版承载数据库。
...
```text
E:\database\clickhouse\runtime\wsl\clickhouse-bsc\
```
...
在 Windows 物理层最终都存放于 E 盘对应的 WSL `ext4.vhdx` 中。
...
```powershell
New-Item -ItemType Directory -Force E:\database\clickhouse\runtime\wsl\clickhouse-bsc
```

如果已有一个临时 Ubuntu 发行版，可导出后重新导入到 E 盘：

```powershell
wsl --export Ubuntu E:\database\clickhouse\runtime\ubuntu-clickhouse.tar

wsl --unregister Ubuntu

wsl --import clickhouse-bsc `
  E:\database\clickhouse\runtime\wsl\clickhouse-bsc `
  E:\database\clickhouse\runtime\ubuntu-clickhouse.tar `
  --version 2
```
...
```powershell
wsl -d clickhouse-bsc
```

再在该 Linux 环境安装 ClickHouse。

> 注意：执行 `wsl --unregister` 会删除原发行版，正式执行前必须确认其中没有其他重要数据。
> 如果已有重要 WSL 环境，应新建专用发行版，不要直接覆盖已有环境。
...
这种 Windows NTFS 跨文件系统访问更适合数据库高频小文件、MergeTree merge 和 metadata 操作。
...
**仅把 `/var/lib/clickhouse` bind mount 到 E 盘还不够。**
...
如果仍位于 C 盘，仍然会持续占用 C 盘。

因此 Docker 方案必须同时满足：

```text
Docker Desktop / WSL data disk → E:
ClickHouse bind mount          → E:
ClickHouse logs                → E:
Backup                         → E:
Export spool                   → E:
```
...
services:
  clickhouse:
    ...
    volumes:
      - E:/codex/clickhouse/data:/var/lib/clickhouse
      - E:/codex/clickhouse/logs:/var/log/clickhouse-server
      - E:/codex/clickhouse/user_files:/var/lib/clickhouse/user_files
      - E:/codex/clickhouse/format_schemas:/var/lib/clickhouse/format_schemas
      - E:/codex/clickhouse/config/config.d:/etc/clickhouse-server/config.d
      - E:/codex/clickhouse/config/users.d:/etc/clickhouse-server/users.d
```

Docker Desktop 的虚拟磁盘/data-root 也必须迁移到 E 盘后才符合本项目约束。

---

# 85. ClickHouse 配置路径约束

如果未来需要自定义路径，统一采用 Linux 内部路径：
...
这些 Linux 路径对应的宿主物理存储必须确认最终位于 E 盘。
...
storage:
  ...
  migration_dir: "E:\\database\\clickhouse\\migration"
  export_spool_dir: "E:\\database\\clickhouse\\export_spool"
  backup_dir: "E:\\database\\clickhouse\\backups"
```
...
```text
ClickHouse/data asset storage is not allowed on C: drive.
Expected root: E:\database\clickhouse
```
...
避免 ClickHouse Merge 时因为空间不足造成异常。
