...
if(isNull(resolved_price) OR NOT isFinite(ifNull(resolved_price,0)),NULL,toString(resolved_price)) AS historical_price_usdt,
if(isNull(resolved_price) OR NOT isFinite(toFloat64(amount_decimal)*ifNull(resolved_price,0)) OR abs(toFloat64(amount_decimal)*ifNull(resolved_price,0))>1e15,NULL,toString(toFloat64(amount_decimal)*resolved_price)) AS historical_value_usdt,
if(isNull(resolved_price) OR NOT isFinite(ifNull(resolved_price,0)),NULL,toString(resolved_price)) AS price_usd,
if(isNull(resolved_price) OR NOT isFinite(toFloat64(amount_decimal)*ifNull(resolved_price,0)) OR abs(toFloat64(amount_decimal)*ifNull(resolved_price,0))>1e15,NULL,toString(toFloat64(amount_decimal)*resolved_price)) AS usd_value,
...
multiIf(
  isNotNull(a.price_usd),toFloat64(a.price_usd),
  token_key='0x55d398326f99059ff775485246999027b3197955',toFloat64(1),
  q.token_address!='',toFloat64(if(q.vwap>0,q.vwap,q.close)),
  p.token_address!='' AND dateDiff('second',p.timestamp_bucket,a.block_time)<=86400,toFloat64(p.price_usd),
  token_key IN ('0x8ac76a51cc950d9822d68b83fe1ad97b32cd580d','0xe9e7cea3dedca5984780bafc599bd69add087d56','0xc5f0f7b66764f6ec8c8dff7ba683102295e16409'),toFloat64(1),
  CAST(NULL AS Nullable(Float64))) AS resolved_price,
