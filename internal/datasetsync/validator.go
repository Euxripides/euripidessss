...
func (v *DuckDBValidator) Validate(ctx context.Context, paths []string, expectedRows int64) (Validation, error) {
	return v.ValidateRange(ctx, paths, expectedRows, 0, 0, nil)
}
...
func (v *DuckDBValidator) ValidateRange(ctx context.Context, paths []string, expectedRows int64,
	...
	statsSQL := fmt.Sprintf(
		"SELECT COUNT(*) AS rows, COUNT(DISTINCT CAST(chain_id AS VARCHAR)||'|'||CAST(block_number AS VARCHAR)||'|'||transaction_hash||'|'||CAST(log_index AS VARCHAR)) AS uniq, COALESCE(MIN(block_number),0) AS min_block, COALESCE(MAX(block_number),0) AS max_block, COUNT(*) FILTER (WHERE NOT (%s)) AS range_violations, COUNT(*) FILTER (WHERE NOT (%s)) AS unexpected_addrs FROM read_parquet(%s)",
		rangeCond, addrCond, pathListSQL(paths),
	)
...
func (v *DuckDBValidator) Merge(ctx context.Context, paths []string, outDir string) (string, error) {
	...
	tmp := merged + ".tmp"
	_ = os.Remove(tmp)
	uniqueKey := `CAST(chain_id AS VARCHAR)||'|'||CAST(block_number AS VARCHAR)||'|'||transaction_hash||'|'||CAST(log_index AS VARCHAR)`
	sql := fmt.Sprintf(`
COPY (
